...
export function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    ...
    `}>
      ...
      {/* Content Section */}
      <div className={`relative z-10 p-6 lg:p-8 md:mr-[42%] md:min-h-[300px] flex flex-col justify-center space-y-4 ${isDarkBg ? 'text-white' : 'text-black'}`}>
        <div className="space-y-2">
          ...
          <p className={`text-sm leading-relaxed font-medium line-clamp-2 transition-colors max-w-md
            ...
            ${isFinished ? "text-emerald-50/70" : isArchived ? "text-indigo-50/70" : isDarkBg ? "text-white/80" : "text-black/70"}
          `}>
            {project.description}
          </p>
        </div>
        ...
        <div className="flex gap-3 pt-2">
          ...
          {project.github_url && (
            <Button
              ...
              variant="ghost"
              size="sm"
              className={`rounded-full border font-bold tracking-tight transition-all duration-500 hover:scale-110 active:scale-95 text-xs h-8 px-4 ${isDarkBg ? 'border-white/10 glass hover:bg-white/10 hover:text-foreground' : 'border-black/10 bg-black/5 hover:bg-black/10'}`}
            >
